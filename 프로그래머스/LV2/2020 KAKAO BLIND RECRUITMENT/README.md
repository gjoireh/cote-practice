...
get_s는 s를 i 단위로 압축하는 함수, get_min은 최소값 구하는 함수다.



그런데 이렇게 제출을 하면 5번테스트케이스에서 실패를 하고 96점인가를 받는다.

(별 문제가 없겠다고 생각 했지만서도 28개 테스트케이스중 27개가 잘 통과되는 것을 보고 놀랐다.)



그런데 테스트케이스 5번이 뭘지가 잘 생각이 안나서 질문하기쪽을 찾아보니
...
문자열의 길이가 1인 경우에는 2로 나눴을때 0이되어 for문이 시작하지 않아서 테스트케이스 5번에서 실패하는 것이다.
...
python의 range는 마지막 요소를 포함하지 않기 때문에 

```python
for i in range(1,1):
```

 이런 식이면 마찬가지로 for문이 시작되지 않기 때문이다.
...
get_s는 문자열 s와 문자열을 압축할 단위를 매개변수로 받는다.
...
이때 이것이 같다면 반복되는 단위를 i번째 인덱스에서 n만큼 뒤로 설정한다.
...
반복되던 단위가 있는 경우에는 그것을 반복 횟수 + 반복 단위로 result에 추가하고
...
그런데 어차피 서류통과가 거의 안되서 코딩테스트 볼일이 매우 드믈기 때문에 별 상관 없기는 하다.
